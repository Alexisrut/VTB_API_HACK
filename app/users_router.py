    ...
    try:
        ...
        if bank_config:
            ...
            if not db_config:
                ...
                # Обновляем существующую конфигурацию, если она неактивна или данные изменились
                if not db_config.is_active or db_config.api_url != bank_config.api_url:
                    db_config.api_url = bank_config.api_url
                    db_config.client_id = bank_config.client_id
                    db_config.client_secret = bank_config.client_secret
                    db_config.requesting_bank = bank_config.requesting_bank
                    db_config.requesting_bank_name = bank_config.requesting_bank_name
                    db_config.redirecting_url = bank_config.redirecting_url
                    db_config.is_active = True
                    await db.flush()
                    logger.info(f"Updated bank config for {bank_user_data.bank_code}")
...
class BankCreateRequest(BaseModel):
    ...
    client_id: Optional[str] = None  # Client ID (если не указан, используется стандартный)
    client_secret: Optional[str] = None  # Client Secret (если не указан, используется стандартный)
...
async def create_bank(
    ...
    try:
        ...
        # Используем credentials из запроса или стандартные
        client_id = bank_data.client_id if bank_data.client_id else settings.VBANK_CLIENT_ID
        client_secret = bank_data.client_secret if bank_data.client_secret else settings.VBANK_CLIENT_SECRET
        ...
        # Если банк уже существует, обновляем его конфигурацию
        if existing_config:
            existing_config.api_url = api_url
            existing_config.client_id = client_id
            existing_config.client_secret = client_secret
            existing_config.requesting_bank = requesting_bank
            existing_config.requesting_bank_name = requesting_bank_name
            existing_config.redirecting_url = redirecting_url
            existing_config.is_active = True
            await db.flush()
            logger.info(f"Updated bank config for {bank_data.bank_code} with URL {api_url}")
        else:
            # Создаем новую конфигурацию банка
            new_bank_config = BankConfigModel(
        ...
        # Валидируем доступность банка (только предупреждение, не блокируем создание)
        validation = await universal_bank_service.validate_bank_exists(
            bank_code=bank_data.bank_code,
            db=db
        )
        ...
        if not validation["exists"]:
            error_msg = validation.get("error", f"Bank {bank_data.bank_code} may not be accessible")
            logger.warning(f"Bank validation warning for {bank_data.bank_code}: {error_msg}")
            # Не блокируем создание банка, продолжаем выполнение
